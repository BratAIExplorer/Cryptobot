...
def migrate():
    ...
    for _, row in pos_df.iterrows():
        ...
        """, (
            ...
            row.get('strategy'),
            row.get('symbol'),
            row.get('buy_timestamp', row.get('entry_date', datetime.utcnow().isoformat())),
            row.get('buy_price', row.get('entry_price', row.get('price', 0))),
            row.get('amount'),
            row.get('status', 'CLOSED'),
            row.get('profit', row.get('realized_pnl', row.get('pnl', 0.0))),
            row.get('exchange', 'MEXC'),
