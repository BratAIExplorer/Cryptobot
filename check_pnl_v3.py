...
def check_pnl():
    ...
    # 2. Open Positions (Unrealized PnL)
    query_open = """
    SELECT 
        strategy,
    ...
    # Merge Stats
    if not df_closed.empty or not df_open.empty:
        # Create master list of strategies
        all_strategies = set(df_closed['strategy'].tolist() + df_open['strategy'].tolist())
        
        print(f"{'STRATEGY':<25} | {'TRADES':<8} | {'OPEN':<6} | {'REALIZED':>10} | {'UNREALIZED':>12} | {'TOTAL PnL':>12}")
        print("-" * 85)
        ...
        for strategy in sorted(all_strategies):
            # Get closed stats
            closed_row = df_closed[df_closed['strategy'] == strategy]
            trades = closed_row['trades'].iloc[0] if not closed_row.empty else 0
            realized = closed_row['realized_pnl'].iloc[0] if not closed_row.empty else 0
            
            # Get open stats
            open_row = df_open[df_open['strategy'] == strategy]
            ...
            print(f"{strategy:<25} | {trades:<8} | {open_count:<6} | ${realized:>9.2f} | ${unrealized:>11.2f} | ${total_pnl:>11.2f}")
